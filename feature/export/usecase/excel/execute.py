...
class ExecuteExcelScoreUpdateUseCase(IExecuteExcelScoreUpdateUseCase):
    ...
    def execute(
        ...
        *,
        ...
        if do_backup:
            try:
                backup_path = self._excel_backup_gateway.create_backup(excel_path)
            except ExcelBackupGatewayError as e:
                raise ExecuteExcelScoreUpdateError(f"バックアップの作成に失敗しました: {e}") from e
        ...
        for student in students:
            ...
            student_score_map[student.student_id] = StudentScoreData(
                name=student.name,
        ...
        # Excelから現在のデータを読み取り
        try:
            excel_cell_table = self._excel_gateway.get_sheet_cells(excel_path, sheet_name)
        except ExcelGatewayError as e:
            raise ExecuteExcelScoreUpdateError(f"Excelファイルの読み込みに失敗しました: {e}") from e
        ...
        if update_values:
            ...
            try:
                ...
                raise ExecuteExcelScoreUpdateError(f"Excelファイルの更新に失敗しました: {e}") from e
