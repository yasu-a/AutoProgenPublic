...
from app.di.repository import get_testcase_config_repository
...
class TestCaseListWidget(QListWidget):
    ...
    def update_data(self):
        self.clear()
        self._testcase_config_summary_lst = get_testcase_list_summary_usecase().execute()
...
class TestCaseListEditWidget(QWidget):
    ...
    def dispatch_action_add(self):
        while True:
            testcase_name, ok = QInputDialog.getText(
                ...
                text=get_testcase_create_new_name_usecase().execute(),
            )
            if not ok:
                return
            ...
            if not testcase_name:
                return
            try:
                get_testcase_create_usecase().execute(testcase_name)
            except UseCaseError:
    ...
    def dispatch_action_copy(self, testcase_id: TestCaseID):
        while True:
            ...
            if not new_testcase_name:
                return
            try:
                get_testcase_copy_usecase().execute(
    ...
    def dispatch_action_delete(self, testcase_id: TestCaseID):
        ...
        if res != QMessageBox.Yes:
            return
        get_testcase_config_repository().delete(testcase_id)
        self.testcase_modified.emit()
