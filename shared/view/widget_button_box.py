...
class ButtonBox(QWidget):
    ...
    def _iter_buttons(self) -> Iterable[QPushButton]:
        yield from self.findChildren(QPushButton)

    def set_button_width(self, v):  # FIXME: not working  TODO: check if it is fixed?
        for b in self._iter_buttons():
            b.setFixedWidth(v)

    def set_button_height(self, v):  # FIXME: not working  TODO: check if it is fixed?
        for b in self._iter_buttons():
            b.setFixedHeight(v)
