...
class StorageFileContentMapper:  # TODO: move to domain service
    ...
    def __setitem__(self, file_relative_path: Path, content_bytes: bytes) -> None:
        # 既存のエントリがある場合は、file_itemだけを更新して新しいエントリを生成
        if file_relative_path in self._cache:
            existing_entry = self._cache[file_relative_path]
            self._cache[file_relative_path] = replace(
                existing_entry,
                file_item=StorageFileItem(
    ...
    def __delitem__(self, file_item: StorageFileItem) -> None:
        # 既存のエントリがある場合は、is_modifiedとis_deletedだけを更新して新しいエントリを生成
        if file_item.relative_path in self._cache:
            existing_entry = self._cache[file_item.relative_path]
            self._cache[file_item.relative_path] = replace(
                existing_entry,
                file_item=file_item,
                is_modified=True,
                is_deleted=True,
