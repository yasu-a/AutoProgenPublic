...
from shared.domain.error import StorageRunCompilerServiceError, CompileToolIOError
...
class StorageRunCompilerService:
    ...
    def execute(
        ...
        if compiler_tool_fullpath is None:
            compiler_tool_fullpath = self._setting_repo.get().compiler_tool_fullpath
        if compiler_tool_fullpath is None:
        ...
        kwargs = dict(
            ...
            # コンパイルのタイムアウト
            timeout=self._setting_repo.get().compile_timeout,
            # コンパイル時のカレントディレクトリ
