...
class StudentStagePathResultEntity:
    # ステージとその結果を管理するクラス
    ...
    def __init__(
        ...
        self._stage_results = stage_results  # mutable
    ...
    def _validate(self):
        ...
        if not isinstance(self._stage_results, OrderedDict):
            raise TypeError(
                f"Expected 'stage_results' to be OrderedDict, "
                f"got {type(self._stage_results).__name__}: {self._stage_results!r}"
            )
    ...
    def __eq__(self, other):
        """IDベースの等価性判定"""
        if not isinstance(other, StudentStagePathResultEntity):
            return False
