...
from shared.domain.error import ManabaReportArchiveIOError
from shared.domain.interface.system import IManabaReportArchiveIO
from shared.domain.value.identifier import StudentID
from util.app_logging import create_logger


class ManabaReportArchiveIO(IManabaReportArchiveIO):
    ...
    def _get_master_excel_relative_path(self, archive_fullpath: Path) -> PurePosixPath:
        # マスタexcelがあるフォルダ
        with zipfile.ZipFile(archive_fullpath, "r") as zf:
    ...
    def _get_master_excel_folder_relative_path(self, archive_fullpath: Path) -> PurePosixPath:
        return self._get_master_excel_relative_path(archive_fullpath).parent
    ...
    def open_master_excel(
        ...
        with zipfile.ZipFile(archive_fullpath, "r") as zf:
            with zf.open(
                    str(self._get_master_excel_relative_path(archive_fullpath)),
                    "r",
            ) as f:
                yield f
    ...
    def validate_master_excel_exists(
        ...
        try:
            self._get_master_excel_relative_path(archive_fullpath)
    ...
    def validate_archive_contents(
        ...
        with zipfile.ZipFile(archive_fullpath, "r") as f:
    ...
    def iter_student_submission_archive_contents(
        ...
        # ^ relative path in StudentEntity folder in archive and fp

        self._logger.info(f"iter_student_submission_archive_contents\n"
                          f" - {student_id!s} {student_submission_folder_name}")

        master_excel_folder_relative_path \
            = self._get_master_excel_folder_relative_path(archive_fullpath)
