...
def _compare_student(s_1: StudentEntity, s_2: StudentEntity):
...
def test_get(sample_students):
    repo = get_student_repository()
    StudentEntity, *_ = sample_students
    student_by_get = repo.get(StudentEntity.student_id)
    assert _compare_student(StudentEntity, student_by_get)
...
def test_list(sample_students):
    ...
    for StudentEntity, student_by_get in zip(sample_students, students_by_list):
        assert _compare_student(StudentEntity, student_by_get)
