...
from app.di.repository import get_student_mark_repository
...
def test_create_and_get(repo, student_id_1):
    ...
    with pytest.raises(ValueError, match="This StudentEntity has not been marked yet"):
        _ = mark.score
...
def test_put_and_get(repo, student_id_1):
    """putとgetの基本テスト"""
    # 採点データを作成
    mark = StudentMarkEntity(student_id=student_id_1, score=85)
    repo.put(mark)
...
def test_put_overwrite(repo, student_id_1):
    """putで上書きするテスト"""
    # 最初のデータ
    mark1 = StudentMarkEntity(student_id=student_id_1, score=85)
    repo.put(mark1)

    # 上書き
    mark2 = StudentMarkEntity(student_id=student_id_1, score=92)
    repo.put(mark2)
...
def test_timestamp_consistency_with_data(repo, student_id_1, student_id_2):
    ...
    # 学生2のデータを作成
    mark2 = StudentMarkEntity(student_id=student_id_2, score=92)
    repo.put(mark2)
...
def test_value_error_on_unmarked_score_access(repo, student_id_1):
    ...
    # 未採点のStudentMarkEntityを作成
    mark = StudentMarkEntity(student_id=student_id_1, score=None)
