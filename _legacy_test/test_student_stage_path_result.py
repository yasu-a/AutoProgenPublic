...
from app.di.repository import get_student_stage_path_result_repository
...
def build_failure_result(student_id_1, student_id_2):
    def _mapper(student_id: StudentID, /, updated=False):
        try:
            reason = {
                (student_id_1, False): "build failed for StudentEntity 1",
                (student_id_1, True): "build failed for StudentEntity 1 updated",
                (student_id_2, False): "build failed for StudentEntity 2",
                (student_id_2, True): "build failed for StudentEntity 2 updated"
            }[student_id, updated]
        except KeyError:
            raise AssertionError(f"unexpected student_id: {student_id}")
...
def compile_success_result(student_id_1, student_id_2):
    def _mapper(student_id: StudentID, /, updated=False):
        try:
            output = {
                (student_id_1, False): "compile ok for StudentEntity 1",
                (student_id_1, True): "compile ok for StudentEntity 1 updated",
                (student_id_2, False): "compile ok for StudentEntity 2",
                (student_id_2, True): "compile ok for StudentEntity 2 updated"
            }[student_id, updated]
        except KeyError:
            raise AssertionError(f"unexpected student_id: {student_id}")
...
def compile_failure_result(student_id_1, student_id_2):
    def _mapper(student_id: StudentID, /, updated=False):
        try:
            reason, output = {
                (student_id_1, False): (
                    "compile error for StudentEntity 1",
                    "error log 1",
                ),
                (student_id_1, True): (
                    "compile error for StudentEntity 1 updated",
                    "error log 1 updated",
                ),
                (student_id_2, False): (
                    "compile error for StudentEntity 2",
                    "error log 2",
                ),
                (student_id_2, True): (
                    "compile error for StudentEntity 2 updated",
                    "error log 2 updated",
                ),
            }[student_id, updated]
        except KeyError:
...
def _create_stage_path_result(student_id: StudentID, stage_path: StagePath,
                              result) -> StudentStagePathResultEntity:
    """単一のステージ結果からStudentStagePathResultEntityを作成"""
    from collections import OrderedDict
    from shared.domain.value.stage import AbstractStage
    from shared.domain import AbstractStudentStageResult
    ...
    return StudentStagePathResultEntity(
        student_id=student_id,
...
def test_put_get_stage_path_result(
    ...
    # put前は存在しない（空のStudentStagePathResultEntityが返される）
...
def test_student_isolation(
    ...
    # s2は存在しない（空のStudentStagePathResultEntityが返される）
...
def test_stage_path_result_methods(
    ...
    """StudentStagePathResultEntityのメソッドをテスト"""
    build_result = get_single_result("build_success_result", student_id=student_id_1)
    compile_result = get_single_result("compile_success_result", student_id=student_id_1)
    ...
    assert statuses[BuildStage()] == StudentStagePathResultEntity.StageStatus.SUCCESS
    assert statuses[CompileStage()] == StudentStagePathResultEntity.StageStatus.SUCCESS
    assert statuses[ExecuteStage(
        TestCaseID("TestCase-1"))] == StudentStagePathResultEntity.StageStatus.UNFINISHED
    assert statuses[
               TestStage(
                   TestCaseID("TestCase-1"))] == StudentStagePathResultEntity.StageStatus.UNFINISHED
